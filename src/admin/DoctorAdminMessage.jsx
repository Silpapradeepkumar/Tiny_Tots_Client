...
import { getDoctorsStatusFalse, getMessages, sentMessage } from "../api/admin";
import { errorToast, successToast } from "../toast";
import Spinner from "../spinner";

function DoctorAdminMessage() {
  ...
  const fetchAPI = async () => {
    try {
      setLoading(true);
      const response = await getMessages(JSON.parse(localStorage.getItem("admin-details"))._id,id);
  ...
  const goToCurrent = `/admin/admin-doctor-message/${id}/${name}`
...
<ul className={breadCrumbContainer}>
        ...
        <li className={`${breadCrubmberClassName}`}>
          <Link to={goToParentDir}>Doctors</Link> <span>/</span>
        </li>
        <li className={`${breadCrubmberClassName}`}>
...
export default DoctorAdminMessage;
